...
--1 trigger.Auto incrementation
CREATE SEQUENCE INCREMENTS
START WITH 1
INCREMENT BY 1
NOMAXVALUE;
...
FOR EACH ROW
BEGIN
   :NEW.EMPLOYEE_ID := INCREMENTS.NEXTVAL;
END;
--2 trigger. Doesn't let add clinic during certain time period
CREATE OR REPLACE TRIGGER T2
BEFORE INSERT 
ON CLINIC
BEGIN
IF(TO_CHAR(SYSDATE,'HH12:MI')  NOT BETWEEN '08:00' AND '22:00') THEN
RAISE_APPLICATION_ERROR(-20222,'Addition of new clinic at this time is not allowed');   
END IF;
END;
--3 trigger.Shows the difference between new and old salary
...
SET SAL = 12000
WHERE EMPLOYEE_ID = 3;
--4 trigger.Patients deletion log: who,by whom and when
...
END;
--5 trigger.Trigger that doesn't allow to delete head doctors
...
--6 trigger.Doesn't allow to make new salary less than old salary
